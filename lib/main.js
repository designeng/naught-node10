...
const json_socket = require('./json_socket');
const spawn = require('child_process').spawn;
const path = require('path');
const DEFAULT_IPC_FILE = 'naught.ipc';
const DEFAULT_PID_FILE = 'naught.pid';
  ...
  var socket = getDaemonMessages(ipcFile, {
    ...
    event: function (msg) {
      if (msg.event === 'Shutdown') {
        ...
        logger.error('Waiting for shutdown already in progress.');
        logger.error(
          'If it hangs, Ctrl+C this command and try it again with --timeout 1',
        );
      } else {
        printDaemonMsg(msg);
      }
...
function workerCountsFromMsg(msg) {
  ...
  return (
    ...
    msg.count.dying +
    ', new_online: ' +
    msg.count.new_online
  );
}

function printDaemonMsg(msg) {
  logger.error(msg.event + '. ' + workerCountsFromMsg(msg));
}
    ...
    event: function (msg) {
      switch (msg.event) {
        case 'ErrorDeployInProgress':
          logger.error('Deploy already in progress. Press Ctrl+C to abort.');
          break;
        case 'Shutdown':
          logger.error(
            "Bootup never succeeded. Check stderr.log and usage of 'online' and 'shutdown' events.",
          );
          process.exit(1);
          break;
        case 'DeployFailed':
          logger.log(
            "Deploy failed. Check stderr.log and usage of 'online' and 'shutdown' events.",
          );
          process.exit(1);
          break;
        case 'Ready':
    ...
    event: function (msg) {
      switch (msg.event) {
        case 'ErrorNoDeployInProgress':
          logger.error('no deploy in progress');
          process.exit(1);
          break;
        case 'Ready':
